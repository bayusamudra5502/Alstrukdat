...
void BacaGARIS(GARIS* L) {
  ...
  POINT awal = MakePOINT(0, 0), akhir = MakePOINT(0, 0);
...
float PanjangGARIS(GARIS L) {
  /* Menghitung panjang garis L : berikan rumusnya
    Rumus yang digunakan adalah : sqrt((p2.y-p1.y) ** 2 + (p2.x-p1.x) ** 2)
    Bisa memanfaatkan rumus Panjang pada ADT Poinr
  */
...
void GeserGARIS(GARIS* L, float deltaX, float deltaY) {
  /* I.S. L terdefinisi */
  /* F.S. L digeser sebesar deltaX ke arah sumbu X dan sebesar deltaY ke arah
   * sumbu Y */
...
boolean IsTegakLurus(GARIS L1, GARIS L2) {
   ...
   */
  return (fabs(Gradien(L1) * Gradien(L2) + 1)) < FLOAT_EPS;
}
...
boolean IsSejajar(GARIS L1, GARIS L2) {
  ...
  return (fabs(Gradien(L1) - Gradien(L2))) < FLOAT_EPS;
}